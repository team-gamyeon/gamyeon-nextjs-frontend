    ...
    return (
      ...
      >
        ...
        <p className="text-muted-foreground text-sm">
          {search ? '검색 결과가 없습니다. 다른 키워드로 검색해보세요.' : '해당 카테고리의 공지사항이 없습니다.'}
        </p>
      </motion.div>
    )
  }
      ...
      <CardContent className="p-0">
        ...
        {notices.map((notice, i) => (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            transition={shouldReduceMotion ? { duration: 0 } : { delay: i * 0.02, duration: 0.15 }}
          ...
          >
            <div className="flex min-w-0 flex-1 items-center gap-3">
              <span
                className={`shrink-0 rounded px-1.5 py-0.5 text-[10px] leading-none font-medium ${CATEGORY_COLORS[notice.category]}`}
              >
                {notice.category}
              </span>
